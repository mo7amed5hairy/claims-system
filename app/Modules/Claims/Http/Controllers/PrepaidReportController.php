...
class PrepaidReportController extends Controller
{
    public function index()
    {
        ...
        // Fetch prepaid claims with their relationships
        $claims = \App\Modules\Claims\Models\PrepaidClaim::with(['payments', 'hospital', 'department', 'entity'])
            ->orderBy('id', 'desc')
            ->get();
        ...
        $entitySubFilters = [
            'تأمين صحي فروع' => ['القاهرة', 'الجيزة', 'رئاسة الهيئة', 'القليوبية', 'مدينة نصر'],
            'الهيئة العامة للتأمين الصحي الشامل' => ['السويس', 'اسماعيلية', 'الأقصر', 'أسوان', 'جنوب سيناء', 'بورسعيد'],
            'قوائم الانتظار' => ['تأمين صحي فروع', 'قوائم انتظار وزارة صحة'],
        ];
