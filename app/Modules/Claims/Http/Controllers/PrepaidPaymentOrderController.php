...
use App\Modules\Claims\Models\PrepaidPaymentOrder as PaymentOrder;
use App\Modules\Claims\Models\PrepaidClaim as Claim;
 ...
 */
class PrepaidPaymentOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('claims::prepaid-payments.index', compact('orders'));
    }
    ...
    public function create()
    {
        ...
        return view('claims::prepaid-payments.create', compact(
            'hospital',
            'department',
            'entities',
            'allHospitals',
    ...
    public function store(Request $request)
    {
        ...
        if (!$waitingListType && !$isHealthDirectorate) {
            ...
            $rules['electronic_invoice_no'] = [
                ...
                \Illuminate\Validation\Rule::exists('claims', 'electronic_invoice_no')->where(function ($query) use ($request) {
                    return $query->where('entity_id', $request->payer_entity_id)->where('is_prepaid', 1);
                }),
            ];
            ...
            $rules['claim_number'] = [
                ...
                'string',
                'max:255',
                \Illuminate\Validation\Rule::exists('claims', 'claim_number')->where(function ($query) {
                    return $query->where('is_prepaid', 1);
                }),
            ];
        ...
        if (!empty($request->electronic_invoice_no)) {
            $claim = \App\Modules\Claims\Models\PrepaidClaim::where('electronic_invoice_no', $request->electronic_invoice_no)->first();
    ...
    public function edit(PaymentOrder $payment)
    {
        ...
        if ($payment->claim_number) {
            $claim = \App\Modules\Claims\Models\PrepaidClaim::where('claim_number', $payment->claim_number)->first();
        } elseif ($payment->electronic_invoice_no) {
            $claim = \App\Modules\Claims\Models\PrepaidClaim::where('electronic_invoice_no', $payment->electronic_invoice_no)->first();
        }
        ...
        return view('claims::prepaid-payments.edit', compact('payment', 'claim', 'entities', 'allHospitals', 'allDepartments', 'searchMode'));
    }
