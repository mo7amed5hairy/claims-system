    ...
    <table id="paymentsTable" class="table" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <div style="font-weight: 600; color: #1e293b; font-size: 13px;">{{ $order->payeeHospital->name ?? '-' }}</div>
                </td>
                <td><span class="badge" style="background: #f1f5f9; color: #475569; padding: 4px 8px; border-radius: 6px; font-size: 12px;">{{ $order->account_type }}</span></td>
                ...
                <td style="text-align: center;">
                    <div style="display: flex; gap: 8px; justify-content: center;">
                        <a href="{{ route('payments.edit', $order->id) }}" class="btn-action" style="background: #eff6ff; color: #3b82f6;" title="تعديل">
                            <i class="fa-solid fa-edit"></i>
                        </a>
                        <form action="{{ route('payments.destroy', $order->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('هل أنت متأكد من حذف أمر الدفع هذا؟');">
                            @csrf @method('DELETE')
