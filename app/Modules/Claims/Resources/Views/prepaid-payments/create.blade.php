...
@section('content')
    ...
    <div class="form-container">
        <div class="form-card">
            <form action="{{ route('prepaid-payments.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" name="taxes" class="form-control" value="{{ old('taxes') }}"
                            min="0">
                    ...
                    <div class="form-group">
                        ...
                        <select name="payer_entity_id" id="entitySelect" class="form-control select2" required>
                            ...
                            @foreach($entities as $entity)
                                ...
                                @endphp
                                <option value="{{ $entity->id }}" {{ $isSelected ? 'selected' : '' }}
                                    data-metadata='@json($entity->metadata)'>{{ $entity->name }}</option>
                            @endforeach
                        </select>
                ...
                @if($showClaimSearch)
                    <div id="claimDetailsSection"
                        style="display: {{ old('claim_number') ? 'block' : 'none' }}; background: #fefce8; border: 1px dashed #f59e0b; border-radius: 6px; padding: 5px 8px; margin: 4px 0;">
                        <h4 style="color: #d97706; margin-bottom: 4px; font-size: 10px;"><i
                                class="fa-solid fa-clipboard-list"></i> بيانات المطالبة</h4>
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;">رقم المطالبة</label>
                                <input type="text" id="claimNumberDisplay" class="form-control" readonly
                                    style="background: #f1f5f9; font-size: 9px;" value="{{ old('claim_number') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;">عدد الفواتير</label>
                                <input type="text" id="claimInvoiceCount" class="form-control" readonly
                                    style="background: #f1f5f9; font-size: 9px;" value="{{ old('invoice_count') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;">قيمة المطالبة</label>
                                <input type="text" id="claimValue" class="form-control" readonly
                                    style="background: #f1f5f9; font-size: 9px;" value="{{ old('claim_value') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;">رقم الفاتورة</label>
                                <input type="text" id="claimElectronicInvoice" class="form-control" readonly
                                    style="background: #f1f5f9; font-size: 9px;" value="{{ old('electronic_invoice_no') }}">
                            </div>
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;"><i class="fa-solid fa-calculator"></i> فواتير
                                    بعد المراجعة</label>
                            ...
                            <div class="form-group">
                                <label class="form-label" style="font-size: 8px;"><i class="fa-solid fa-coins"></i> المبلغ بعد
                                    المراجعة</label>
                ...
                <div class="form-row" style="gap: 4px; margin: 0; margin-bottom: 4px;">
                    <div class="form-group" id="branchContainer" style="display: none; margin: 0;">
                        <label class="form-label" style="font-size: 9px;"><i class="fa-solid fa-layer-group"></i>
                            الفروع</label>
                    ...
                    <div class="form-group" id="subContainer" style="display: none; margin: 0;">
                        <label class="form-label" id="subLabel" style="font-size: 9px;"><i
                                class="fa-solid fa-map-marker-alt"></i> المحافظات</label>
                    ...
                    <div class="form-group" id="lawsContainer" style="display: none; margin: 0;">
                        <label class="form-label" style="font-size: 9px;"><i class="fa-solid fa-file-lines"></i>
                            المستفيدين</label>
                ...
                <div class="form-group" style="margin-bottom: 6px;">
                    <label class="form-label" style="font-size: 9px;"><i class="fa-solid fa-note-sticky"></i>
                        ملاحظات</label>
                    <textarea name="notes" class="form-control" rows="3"
                        style="min-height: 50px; height: 50px; font-size: 10px; resize: vertical;"
                ...
                <div class="form-section"
                    style="padding: 10px !important; margin: 8px 0; border: 1px dashed #cbd5e1; border-radius: 6px; background: #f8fafc;">
                    ...
                    <div class="drop-zone" id="dropZone"
                        style="padding: 15px !important; margin-bottom: 8px; border: 2px dashed #94a3b8; border-radius: 8px; text-align: center; cursor: pointer; background: #fff; transition: all 0.3s;">
                        <div class="drop-zone-content">
                            <i class="fa-solid fa-cloud-arrow-up" style="font-size: 24px; color: #64748b;"></i>
                            <p class="drop-zone-text" style="font-size: 13px; margin: 5px 0; color: #475569;">اسحب وأفلت
                                الملفات هنا أو اضغط للاختيار</p>
                            <p class="drop-zone-hint" style="font-size: 11px; margin: 0; color: #94a3b8;">يمكنك رفع ملفات
                                متعددة (الحد الأقصى 10 ميجا لكل ملف)</p>
                        </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function searchClaim(claimNumber) {
                ...
                $.ajax({
                    url: '{{ route("prepaid-payments.search-claims") }}',
                    method: 'GET',
