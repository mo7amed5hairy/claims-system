            ...
            <form action="{{ route('claims.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row-4">
                    ...
                    <div class="form-group" id="branchContainer" style="display: none;">
                        ...
                        <input type="hidden" name="branch" id="branchHidden"
                            value="{{ old('branch', session('flow_options.branch', '')) }}">
                    </div>
                    ...
                    <div class="form-group" id="subContainer" style="display: none;">
                        <label class="form-label" id="subLabel"><i class="fa-solid fa-map-marker-alt"></i> المحافظات /
                            المواقع</label>
                        <select id="subSelect" class="form-control select2">
                            <option value="">اختر المحافظة / الموقع</option>
                        </select>
                        <input type="hidden" name="location" id="subHidden"
                            value="{{ old('location', session('flow_options.location', '')) }}">
                    </div>
                    ...
                    <div class="form-group" id="lawsContainer" style="display: none;">
                        ...
                        <select id="lawsSelect" class="form-control select2">
                            <option value="">اختر المستفيد</option>
                        </select>
                        <input type="hidden" name="beneficiary" id="lawsHidden"
                            value="{{ old('beneficiary', session('flow_options.law', '')) }}">
                    </div>
    ...
    @section('scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                // ملء القوائم من metadata
                function fillBranchOptions(metadata) {
                ...
                branchSelect.on('change', function () {
                    ...
                    branchHidden.val(branchVal);

                    const metadata = entitySelect.find('option:selected').data('metadata');
                    fillSubOptions(metadata, branchVal);
                });

                // عند اختيار المحافظة / الموقع
                subSelect.on('change', function () {
                    const subVal = $(this).val() || '';
                    subHidden.val(subVal);

                    const metadata = entitySelect.find('option:selected').data('metadata');
                    fillLawsOptions(metadata, subVal);
                });

                // عند اختيار المستفيد / القانون
                lawsSelect.on('change', function () {
                    lawsHidden.val($(this).val() || '');
                });

                // ==== تهيئة الصفحة عند التحميل (إذا موجود values من السيشن أو old) ====
                const initialEntity = entitySelect.val();
                const initialBranch = branchHidden.val();
                const initialSub = subHidden.val();
                const initialLaw = lawsHidden.val();
